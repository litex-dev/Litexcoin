...
*Setup instructions for a gitian build of LTX using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the LTX
...
Clone the git repositories for ltx and gitian and then checkout the ltx version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/ltx-crypto/ltx.git
cd ltx
...
Follow the instructions in [doc/release-process.md](release-process.md) in the ltx repository
...
To build LTX (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the ltx repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/ltx/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/ltx-crypto/ltx
...
For example:
```bash
URL=https://github.com/litexcoin/ltx.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit ltx=${COMMIT} --url ltx=${URL} ../ltx/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit ltx=${COMMIT} --url ltx=${URL} ../ltx/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit ltx=${COMMIT} --url ltx=${URL} ../ltx/contrib/gitian-descriptors/gitian-osx.yml
```
...
and follow the steps in the build process as normal.
    ...
    gpg: skipped "ltxcoin": secret key not available

When you execute `gsign` you will get an error from GPG, which can be ignored. Copy the resulting `.assert` files
in `gitian.sigs` to your signing machine and do

```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/ltx-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/ltx-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/ltx-build.assert
```
...
[ltx/gitian.sigs](https://github.com/ltx-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
